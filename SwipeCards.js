...
export default class SwipeCards extends Component {
  ...
  renderStack() {
    ...
    return cards.map((card, i) => {
      ...
      return <Animated.View key={card[this.props.cardKey]} style={style}>{this.props.renderCard(card, this.state.pan)}</Animated.View>;
    });
  }
  ...
  renderCard() {
    ...
    let zIndex = pan.x.interpolate({ inputRange: [-50, 0, 50], outputRange: [0, 1, 0] });
    let scale = enter;

    let animatedCardStyles = { transform: [{ translateX }, { translateY }, { rotate }, { scale }], opacity, zIndex };
